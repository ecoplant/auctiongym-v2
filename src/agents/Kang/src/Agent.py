...
""" class SAC(Agent):
    def __init__(self, rng, name, item_features, item_values, context_dim, buffer, config):
        ...
        self.critic = QNetwork(self.context_dim+2+1, config['hidden_dim'], action_dim = 10).to(self.device)
        self.critic_target = copy.deepcopy(self.critic)
        self.critic_optim = optim.Adam(self.critic.parameters(), lr = config['lr'])
        ...
        if self.policy_type == "GaussianPolicy":
            if self.automatic_entropy_tuning is True:
                ...
                self.alpha_optim = optim.Adam([self.log_alpha], lr = config['learning_rate_a'])

            self.policy = GaussianPolicy(config.state_dim, config.action_dim, config.hidden_size, action_space=None).to(self.device)
            self.policy_optim = optim.Adam(self.policy.parameters(), lr = config['learning_rate_a'])
