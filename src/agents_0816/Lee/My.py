...
class My(Agent):
    ...
    def bid(self, state):
        if self.step < 2:
            ...
            last_win = self.buffer.wins[-1]
            second_last_win = self.buffer.wins[-2]
            last_reward = self.buffer.rewards[-1]
            second_last_reward = self.buffer.rewards[-2]
            if last_win == 1:
                if second_last_win == 1:
                    avg_reward = 2 * (last_reward + second_last_reward) / (last_reward * second_last_reward + 1e-6) # add 1e-6 to avoid 'divide by zero' error
                    bidding = avg_reward
                else:
                    bidding = last_reward
            else:
                if second_last_reward == 1:
                    bidding = second_last_reward
                else:
                    bidding = 0.3
