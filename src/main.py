...
if __name__ == '__main__':
    ...
    sample_size = min(len(buffer.states), 800)
    states, item_inds, biddings, rewards, next_states, dones = agent.buffer.sample(sample_size)
    temp = np.concatenate([np.tile(np.linspace(0,budget,40),(20,1)).reshape(-1,1),
                           np.tile(np.linspace(0, horizon, 20),(1,40)).reshape(-1,1)], axis=1)
