    ...
    def update(self):
        ...
        for i in range(self.num_grad_steps):
            ...
            with torch.no_grad():
                ...
                target_biddings = self.actor_target(torch.cat((torch.Tensor(next_contexts), torch.Tensor(next_resources), torch.Tensor(next_biddings)), dim=1).to(self.device))
                noisy_target_biddings = target_biddings + torch.clamp(torch.randn_like(target_biddings) * noise_parameter, min=-noise_clip, max=noise_clip).to(self.device)

                x = torch.cat((torch.Tensor(next_contexts).to(self.device), torch.Tensor(next_resources).to(self.device), noisy_target_biddings), dim=1)
