        ...
        with torch.no_grad():
            if self.exploration_strategy=='Eps-Greedy':
                if self.rng.uniform(0, 1) < self.eps:
                    ...
                    bid = self.item_values[item] * self.rng.random() * 1.5
                else:
...
class TD3(Agent):
    ...
    def get_uncertainty(self, len):
        ...
        # main 에서 line 247 ~ 277 까지를 사용해 Q value 를 출력해봐라    
        # horizon 줄여서 학습 되는지 확인
        # episode 갯수도 늘려서 학습 되는지 확인해보자
        # generate 되는 state 의 평균을 봐야 현재 state 가 좋은지 나쁜지 판단가능. 지금은 그게 state value 로 판단이 될거라 생각했는데, 문제는 policy가 update 되기 때문에 stae value 가 의미가 없어짐.
        # naive 하게 이전에 직면한 state 들을 기억해라?
        # bidding 을 skip 하는 것도 action 에 포함시켜봐라

        # step, budget 이 줄면 value 도 줄어야함
        # 1. horizon 을 줄여봐
        # 2. action 중 bidding 을 0, 0.1, 0.2 ... 1.0 과 같이 discretize?
        # 3. contextual bandit 으로 풀어봐라?
            # budget 도 context 로 보고, reward (episode 끝날때까지의) 를 보고 bandit. Episode 끝날때마다 update. 

        # bidding 에서 grid search 할 떄 지금 100 정도로 나누는데, 10정도만 나눠도 되지 않을까?
        # Q value 를 찍어봐라. 

        # context dim 3 으로 줄여보자
        # item 갯수는 5개정도로 하자
        # budget 을 2정도로, horizon 을 4정도로
        # discretize 는 10정도로