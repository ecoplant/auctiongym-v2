...
class DQN(Agent):
    ...
    def set_exploration_param(self, ep):
        ...
        if self.exploration_strategy=='Eps-Greedy':
            ...
            if not self.min_achieved:
                if self.eps == self.eps_min: 
                    self.min_achieved = True
                    print("eps min is achieved")
        elif self.exploration_strategy=='Noise Injection':
    ...
    def update(self, t):
        ...
        for i in range(self.num_grad_steps):
            ...
            with torch.no_grad():
                ...
                n_values_search = int(20*np.max(self.item_values))
                b_grid = np.linspace(0, 1.5*np.max(self.item_values), n_values_search).reshape(-1,1)
                x = torch.Tensor(np.hstack([np.tile(next_contexts, (1, n_values_search)).reshape(-1, self.context_dim),
                                            np.tile(self.items[next_items],((1, n_values_search))).reshape(-1, self.context_dim),
                                            np.tile(next_resources, (1, n_values_search)).reshape(-1, 2), np.tile(b_grid, (self.batch_size, 1))])).to(self.device)
                if self.exploration_strategy=='Noisy Network':
